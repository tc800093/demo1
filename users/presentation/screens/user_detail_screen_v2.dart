            ...
            child: Column(
              ...
              children: [
                ...
                CustomExpansionCard(
                  ...
                  child: _ServiceHistorySection(userId: user.userId ?? ''),
                ),
                SizedBox(height: AppSizes.height(2.5)),
                ...
                CustomExpansionCard(
                  ...
                  child: _RefuelHistorySection(userId: user.userId ?? ''),
                ),
...
// =============================================================================
// Service History Section Widget & Bottom Sheet
// =============================================================================

class _ServiceHistorySection extends StatelessWidget {
  final String userId;
  const _ServiceHistorySection({required this.userId});

  void _showAddServiceBottomSheet(BuildContext context, List<DeviceModel> devices) {
    final theme = Theme.of(context);
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      CustomTextfieldWidget(
                        ...
                        validatorFunction: (val) => val == null || val.isEmpty ? 'Required' : null,
                      ),
                      const SizedBox(height: 16),
                      CustomTextfieldWidget(
                        ...
                        child: Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              Row(
                                children: [
                                  Icon(Icons.calendar_today_outlined, color: theme.iconTheme.color),
                                  const SizedBox(width: 8),
                                  Text(
                      ...
                      CustomButtonWidget(
                        ...
                        onPressedFunction: () {
                          if (formKey.currentState!.validate()) {
                            final dev = devices.firstWhere((d) => d.deviceId == selectedDeviceId);
                            ...
                            context.read<UserDeviceProvider>().addServiceRecord(newRecord);
                            Navigator.of(ctx).pop();
    ...
    return Consumer<UserDeviceProvider>(
      builder: (context, udp, _) {
        ...
        for (var device in udp.devices) {
          allRecords.addAll(udp.getServiceRecordsForDevice(device.deviceId ?? ''));
        }
        ...
        return Column(
          ...
          children: [
            if (allRecords.isEmpty) ...[
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final dateStr = DateFormat('dd MMM yyyy').format(record.serviceDate);
                  return Container(
                    margin: const EdgeInsets.only(bottom: 12),
            ...
            if (udp.devices.isNotEmpty)
              OutlinedButton.icon(
                onPressed: () => _showAddServiceBottomSheet(context, udp.devices),
                icon: const Icon(Icons.add),
...
// =============================================================================
// Refuel History Section Widget & Bottom Sheet
// =============================================================================

class _RefuelHistorySection extends StatelessWidget {
  final String userId;
  const _RefuelHistorySection({required this.userId});

  void _showAddRefuelBottomSheet(BuildContext context, List<DeviceModel> devices) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
                    ...
                    children: [
                      ...
                      CustomButtonWidget(
                        ...
                        onPressedFunction: () {
                          if (formKey.currentState!.validate()) {
                            final dev = devices.firstWhere((d) => d.deviceId == selectedDeviceId);
                            final newRecord = RefuelRecord(
                              id: DateTime.now().millisecondsSinceEpoch.toString(),
                              deviceId: selectedDeviceId!,
                              deviceName: dev.deviceName ?? 'Device',
                              refuelDate: selectedDate,
                              fuelAmountLiters: double.parse(fuelAmountController.text.trim()),
                              cost: double.parse(costController.text.trim()),
                              notes: notesController.text.trim(),
                            );
                            context.read<UserDeviceProvider>().addRefuelRecord(newRecord);
                            Navigator.of(ctx).pop();
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<UserDeviceProvider>(
      builder: (context, udp, _) {
        ...
        final allRecords = <RefuelRecord>[];
        for (var device in udp.devices) {
          allRecords.addAll(udp.getRefuelRecordsForDevice(device.deviceId ?? ''));
        }
        allRecords.sort((a, b) => b.refuelDate.compareTo(a.refuelDate));
          ...
          children: [
            if (allRecords.isEmpty) ...[
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  final record = allRecords[index];
                  final dateStr = DateFormat('dd MMM yyyy').format(record.refuelDate);
                  return Container(
                    margin: const EdgeInsets.only(bottom: 12),
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Text(
                              '${record.fuelAmountLiters.toStringAsFixed(1)} Liters',
                              style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.bold),
                            ),
                            Text(
                              '₹${record.cost.toStringAsFixed(0)}',
            ...
            if (udp.devices.isNotEmpty)
              OutlinedButton.icon(
                onPressed: () => _showAddRefuelBottomSheet(context, udp.devices),
                icon: const Icon(Icons.add),
