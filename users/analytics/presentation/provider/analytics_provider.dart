...
class AnalyticsProvider extends ChangeNotifier {
  ...
  Future<void> fetchAnalyticsMethod({
    ...
    try {
      final isSameDay = params.fromDate == params.toDate;
      ...
      await result.fold(
        ...
        (success) async {
          ...
          if (model.generatorAnalytics!.generatorStartCount == 0 &&
              model.msebAnalytics!.outageCountPerDay == 0) {
            _generateUseage = 0.0;
            _mainPowerUsage = 1.0;
          } else {}
          ...
          AppLogger().debug("main power useage ${_generatorCount.length}");
          ...
          if (isSameDay) {
            ...
            double value = calculateRuntimePercentage(
              totalRuntimeHours:
                  model.msebAnalytics!.downtimeDurationMinutes ?? 0.0,
              fromDate: DateTime.parse(params.fromDate),
              toDate: DateTime.parse(params.toDate),
            );

            AppLogger().debug("the value given is $value");
          }
        },
      );
      notifyListeners();
  ...
  List<RuntimeSegment> buildSegments(
    ...
    for (int i = 0; i < events.length - 1; i++) {
      if (events[i].eventType.toString().toUpperCase() == start &&
          events[i + 1].eventType.toString().toUpperCase() == end) {
        final start = events[i].eventTime!;
        final end = events[i + 1].eventTime!;
